...
python3 ${HF_WORKFLOWS}/ugc-product-video/scripts/transcribe_words.py output/final.mp4 -o output/words.json

# 2. script-aligned grouping, Montserrat 52; re-anchor to BOTTOM (~0.15·H)
python3 ${HF_WORKFLOWS}/ugc-product-video/scripts/group_captions.py output/words.json -o output/segments.json \
  --video output/final.mp4 --script output/script.txt      # exits 3 if a caption word is not in the script
python3 ${HF_WORKFLOWS}/ugc-product-video/scripts/make_captions.py output/segments.json -o output/captions.ass --font "Montserrat" --size 52
python3 - <<'PY'
...
python3 ${HF_WORKFLOWS}/ugc-product-video/scripts/make_captions.py output/hook_seg.json -o output/hook.ass
python3 - <<'PY'
