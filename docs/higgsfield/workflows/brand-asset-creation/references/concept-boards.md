...
Render all options in editable HTML with `${HF_WORKFLOWS}/brand-asset-creation/scripts/brandkit.py preview`, then publish them per `inline-widgets.md` (board PNG screenshots shown inline + editable HTML links). Do not generate image-model duplicates and do not send filenames alone.
...
Load `inline-widgets.md`. Reserve PNG/HTML uploads first, then render, screenshot, and PUT every board in the same sandbox batch, show each option's confirmed PNG inline, and provide the confirmed HTML URL as the editable download. For 2–3 options, stack the complete option blocks vertically in one message.
...
3. Submit the three enhanced prompts through `generate_image_batch`, one `{index, params}` request per candidate with `count: 1`, model `recraft_v4_1`, `model_type: "vector"`, the selected logo color subset in `colors`, and the selected background in `background_color`. Use the one, two, or three logo colors the concept needs; never pad to three. Exceed three only when the user explicitly requested more.
...
When the user selects a logo, immediately call the Brandkit state script's `approve_logo` action with the exact Recraft SVG asset, name, fingerprint, and any explicitly requested variants. A logo-only request is complete at this point; typography is not mandatory.
...
- Render the real brand name and short sample copy through `${HF_WORKFLOWS}/brand-asset-creation/scripts/brandkit.py preview`.
...
Render a combined board only when all three slots are approved and the user requested a full identity/Brandkit or asks to see them together. Otherwise show the available individual review boards. Use `${HF_WORKFLOWS}/brand-asset-creation/scripts/brandkit.py preview` with one `stage: "essential"` review containing the approved logo, palette, and type slots.
