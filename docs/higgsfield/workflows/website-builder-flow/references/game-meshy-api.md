...
> **STOP — check for the native tool first.** If `higgsfield_generate_3d`
> (supercomputer) or `generate_3d` (MCP) is loaded, do NOT use this raw API and
...
> `higgsfield_3d_animation_actions`), stage [4] = the same local merge script.
...
- Auth: `Authorization: Bearer <msy_... key>` (ask the user for their key;
  never hardcode it into committed files).
...
present, alphaMode OPAQUE) and zip before `higgsfield_upload` (raw `.glb`
