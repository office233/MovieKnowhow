...
This is THE design source for every `type: "website"` build. (NEVER for
`type: "app"` builds — apps have no wow pipeline: Quanta is the design and the
shipped starter template is adapted in place per `references/app-flow.md`.)
There are no fixed design templates to
...
the scroll-scrub journey in `references/scroll-scrub.md` — and
...
does. Never use "it's minimal" as cover to ship a plain page or downgrade the
animated website to a soft effect.
...
`job_display` → use the result**. If a param/model is rejected, call
`models_explore` and use what it reports.

- `generate_image` — `gpt_image_2` (general / art-directed), `nano_banana_pro`
  (photoreal + reference-image driven, e.g. try-ons / product normalization).
- `generate_video` — `seedance_2_0` (seamless loops + short films).
