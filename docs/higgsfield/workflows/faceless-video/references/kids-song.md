...
3. **Generate the SONG** — one indexed `generate_audio_batch` item with `seed_audio`,
   **prompt-only: NO `voice_id`, NO `voice_type`** (the singing voice is described inside
   the prompt). Wait with `jobs_wait`, show the exact song once, and log it
   logged as `song-001` in `work/manifest.json`; a retry reuses that label and never
   runs two variants at once.
   ...
   spirit as the clip retry ladder — never burn credits in a loop). Post the
   song as a notification (gates post-and-proceed).
4. **Style key + asset roster** (Phases 1–2, unchanged; may run in parallel
   with step 3). Roster = the MAIN CHARACTER + friends the lyrics name
...
6. **Generate the 12 (or 6) blocks** (Phase 4 as usual, submission chunks ≤6): prompts
   ...
   diegetic-only (claps, hops — the song is added in post).
7. **Assemble with `--song`** (Phase 6):
   ```
   ${HF_WORKFLOWS}/faceless-video/scripts/assemble_final.sh --out final.mp4 --blocks N --manifest clips.txt --song song.wav
   ```
   ...
   (`"song":true`).
8. Deliver ONE `final.mp4`. Phase 8/9 as usual (billing `block_count` = N).
...
**GATE THE PROMPT BEFORE YOU SUBMIT IT — the laws below are machine-checked:**
```
python3 ${HF_WORKFLOWS}/faceless-video/scripts/validate_song_prompt.py --prompt song_prompt.txt
```
Exit 1 lists exactly what drifted (missing steady-beat clause, a banned tempo word, an
untested BPM, off-meter lyric lines) — fix those and rerun until it passes. Never submit an
ungated song prompt and never "try it and see": every "the rhythm is awful" report
(2026-07-25 v2, 2026-07-29 U25) came from a prompt that had wandered off this skeleton, and
each retry costs a full song generation.
