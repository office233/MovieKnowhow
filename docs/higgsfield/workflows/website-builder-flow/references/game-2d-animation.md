...
> **Game asset reference.** Use for ANY animated 2D asset —
> characters, objects, effects — delivered as a spritesheet. The pipeline:
> key-pose image → `generate_video` (**`seedance_1_5`**) → frame extraction
...
| 2 | Key-pose image | `generate_image` → `flux_2` | full-body pose at the action's mid/peak phase |
| 3 | Animation video | `generate_video` → **`seedance_1_5`** | 4 s, 720p, AR = key-pose ratio (explicit); loop: start = end frame |
...
- **The video model is `seedance_1_5` exactly.** Not `seedance_2_0`, not the
  current default of `generate_video`. Verify availability via
  `models_explore` if a call errors, but do not silently substitute a model.
...
**How:** `generate_image` with model `flux_2`, the source image as media
...
## Stage 3 — Video generation (`seedance_1_5`)
...
Each selected frame: upload (`media_upload` → confirm, per the
image-generation skill's general mechanics) → `image_background_remover` with
`media_type: image` → download the transparent PNG.
