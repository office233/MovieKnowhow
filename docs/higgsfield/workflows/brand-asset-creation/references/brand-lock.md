...
- One approved display/body typography system and rendered specimen
...
- Upload each authoritative file once and reuse the same upload ID.
...
## Chat-local approval persistence

The working Brand Lock may contain draft concept language, but final approvals live in this conversation's confirmed state snapshot, restored to `.brandkit/state.json` for each sandbox batch per `handoff.md`.
...
- Do not store Brandkit approval state in project files, user memory, or any cross-chat store.
