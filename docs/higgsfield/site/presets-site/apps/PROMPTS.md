...
**Verbatim prompts in this section: 0.** Higgsfield Apps are prompt-free one-click presets (upload media, optionally pick an option or preset, click Generate), and no app page exposes a prompt string in its HTML or page data. The only free-text inputs are the ASMR Promo script and the AI Meme Generator character prompt and captions, and the pages give no examples for either.

Below, grouped by use-case, is **what to upload** for each app group, as a stand-in for prompts.

## Cinematic film scene (pre-production and finishing)

Input: one strong keyframe (a Soul/Nano Banana still). Run Shots, Angles and Zooms to get coverage of the same scene, What's Next? for story beats, Relight and Color Grading to match the look, and Expand image to reach 21:9 or 16:9. Then animate each frame with image-to-video.

| App | Local file | Cost |
|---|---|---|
| Shots | [shots.md](shots.md) | — |
| Angles 2.0 | [angles.md](angles.md) | — |
| Zooms | [zooms.md](zooms.md) | — |
| What's Next? | [whats-next.md](whats-next.md) | — |
| Behind the Scenes | [behind-the-scenes.md](behind-the-scenes.md) | Image: 2 credits (template `bts-image`) / Video: 6 credits (template `bts-video`) |
| Expand image | [expand-image.md](expand-image.md) | Image 2 |
...
Input: a clear, front-lit, unobstructed face or character image plus the target photo or video. One face per job. Swap in a fixed character across many scenes (Character Swap 2.0) or videos (Recast) to keep one identity throughout a film or ad set.

| App | Local file | Cost |
|---|---|---|
| Character Swap 2.0 | [character-swap.md](character-swap.md) | Image: 2 credits (template `face-swap`) |
| Face Swap | [face-swap.md](face-swap.md) | Image: 2 credits (template `face-swap`) |
| Face Swap | [video-face-swap.md](video-face-swap.md) | Image: 2 credits (template `face-swap`) |
| Recast | [recast.md](recast.md) | Video: 2 credits (template `face-swap`) |
| Headshot Generator | [ai-headshot-generator.md](ai-headshot-generator.md) | Image 1 |
| Commercial Faces | [commercial-faces.md](commercial-faces.md) | Image: 2 credits (template `commercial-faces`) |
...
Input: one clean product photo (ideally a cutout from Background Remover). Chain a hero effect (Bullet Time, Giant Product, Billboard, Volcano…) with a detail shot (Macroshot) and finish with Packshot as the end card. ASMR Promo takes your own script text.
...
| Video Background Remover | [video-background-remover.md](video-background-remover.md) | Video 1 |

## UGC

Input: a selfie or creator photo. Use ASMR Host or Mukbang for creator-style formats. Check hooks of 15 s or less with Virality Predictor before posting.
...
Input: one well-lit portrait with the subject centered (waist-up) on a simple background. Most trend apps offer a cheap image mode (1-2 credits) and a video mode (15-26 credits). Preview in image mode first, then pay for the video.
...
| Micro-Beasts (Surrounded by Animals) | [surrounded-by-animals.md](surrounded-by-animals.md) | Image: 2 credits (template `face-swap`) |
| Japanese Show | [japanese-show.md](japanese-show.md) | Image: 2 credits (template `japanese-show-image`) |
| Outfit Vending | [vending-machine.md](vending-machine.md) | Image: 2 credits (template `outfit-vending-image`) |
| Breakdown | [breakdown.md](breakdown.md) | Image: 2 credits (template `breakdown-image`) |

## Transitions

Input: two clips or photos, each video 5 s or less (Transitions). The output aspect ratio follows clip 1, and the transition adds 5 s (15 s max).

| App | Local file | Cost |
|---|---|---|
| Transitions | [transitions.md](transitions.md) | Video: 10 credits (template `transitions`) |
| Sticker Match Cut | [sticker-matchcut.md](sticker-matchcut.md) | Video: 25 credits (template `sticker-matchcut-video`) |
| ClipCut | [clipcut.md](clipcut.md) | — |

## Music video

Input: a face or character photo plus an outfit image and a music track (Urban Cuts: 10 s, cuts land on the beat, 9:16 default), or a single photo (Rap God generates lyrics, beat and flow).
