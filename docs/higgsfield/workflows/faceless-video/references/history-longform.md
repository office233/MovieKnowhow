...
**Catalog card: "Watercolor Chronicle"** (`0029f935-be9e-46c7-a5d8-a4e0f81d49c8`).
...
together with the card's own resolved `media_id` — including on hands-off long-form runs
where the style auto-locked. Never build this key from prose alone.

- https://cdn.higgsfield.ai/youtube_faceless_preset_image/3898424f-d922-486d-8731-7d86ba35d2d5.webp
...
2. For every recurring character, list the eras they appear in and WHAT
   ...
   **IDENTITY CHAIN (mandatory):** the FIRST incarnation is generated from
   ...
   [style anchor, base variant job_id], role `image_references`) with the
...
- **Generate chapter by chapter** (`generate_*_batch` chunks of at most 12 inside a
  chapter): collect a
  chapter's blocks, then move on — a failed block only stalls its chapter.
...
- **Voice:** documentary narrator — measured but warm; the standard voice
  ...
  and the 20–23-word density apply to every block including chapter openers.
