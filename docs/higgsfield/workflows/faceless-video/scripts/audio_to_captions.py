#!/usr/bin/env python3
"""WORD-TIMING tool. In `faceless-video` this exists for ONE job: building the
Picture Story FRAME TIMELINE from Whisper's word clock (`--json words.json`).

IT IS NOT THE CAPTION PATH OF THIS SKILL. Captions belong to the `video-subtitler` skill,
which ships its own copy of this file plus the burners, the fonts and the gates. Using
this script here to produce an .srt, or feeding its output into any burner from this
skill, is a hard failure of Phase 7 — that is exactly how dev 2026-07-29 ended up with
three-line paragraphs across the middle of the frame.
...
def _timed_script_span(
    ...
    *,
    ...
    cursor = start
    # `weights` is derived from `script_tokens` immediately above. Avoid
    # `strict=True` so the preinstalled script also runs on Python 3.9.
    for index, (token, weight) in enumerate(zip(script_tokens, weights)):
        token_end = (
            end
...
def align_words_to_script(
    ...
    *,
    ...
    `span_floor` / `span_ceiling` bound an INTERPOLATED span when the STT dropped
    words at the edge of this text, so a swallowed opener cannot be spread back
    across preceding silence. Callers aligning one block/beat at a time pass that
    unit's speech window (see the `video-subtitler` skill's copy of this script).
    """
