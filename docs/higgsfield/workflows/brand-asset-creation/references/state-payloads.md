...
Write these complete JSON shapes into the sandbox with a quoted heredoc (`cat > brandkit/input.json <<'JSON' … JSON`). Replace values only. Keep asset objects, arrays, key names, and nesting unchanged.
...
```json
{
  ...
  "logo": {
    ...
    "geometry_fingerprint": "replace-with-exported-fingerprint"
  }
}
```

`logo.asset` is always an object, never a URL string.
...
```json
{
  ...
  "brandbook_element": {
    ...
    "asset": {
      "id": "replace-with-upload-id",
      "type": "file_input",
      "url": "https://replace-with-exact-approved-poster.svg"
    }
  }
}
```
