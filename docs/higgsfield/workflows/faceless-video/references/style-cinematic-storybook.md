...
`references/kids-styles.md §Kids music bed`), but the MOOD is **dark-enchanted
...
Import each with `media_import_url`, pass ALL returned media_ids as
`image_references` into ONE `seedream_v5_pro` style-key call with the FORMULA
...
Motion-video mechanics = the History channel: N×10s blocks, `minimax_h3`,
...
optional post-delivery Topaz handling in Phase 8, `validate_motion_script.py` Gate 3. It is a look + tone + music
