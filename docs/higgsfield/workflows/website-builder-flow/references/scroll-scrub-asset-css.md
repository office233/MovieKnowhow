...
`scroll-scrub` template already ships this exact stylesheet at
`app/src/components/scroll-scrub/scroll-scrub.css`. Restyle it in place from the
design brief; do not re-materialize it from here.

This file is the ENGINE SOURCE OF RECORD, kept in sync with the template. Read
it only when the task is to CHANGE the engine itself, or to repair a site
scaffolded before the template existed. Read `references/scroll-scrub.md` first
either way. It is a layout contract — replace composition values through the
design brief, and never add generic shared CTA chrome.
