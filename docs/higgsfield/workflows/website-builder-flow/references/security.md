...
#### 3. Live Data Blast Radius (Single D1)

**Attack:** There is one deploy and one D1 database — the live production data. A migration, seed script, or ad-hoc write run "just to test" executes directly against real user data, and a destructive change cannot be caught in a staging copy first.

**Mitigation:**
- Never use real user data for testing; exercise risky queries locally before deploying
- Prefer additive migrations (`CREATE TABLE IF NOT EXISTS`, `ADD COLUMN`)
- Treat destructive migrations (`DROP`, destructive `UPDATE`, backfills) as live production changes requiring explicit user approval
...
2. **Don't assume D1 is multi-tenant by default.** Each website gets its own D1 database; different websites never share one. The risk is blast radius, not cross-tenant leaks: the website's single database IS live production data (see pattern 3).
...
// GOOD -- use website_secrets (set) when supported, or website settings
...
export function applySecurityHeaders(response: Response): Response {
  ...
  // Framing: Supercomputer Design mode renders the live deployed app
  ...
  // DENY would blank that iframe. We deliberately DO NOT set X-Frame-Options and
...
`DENY`/`SAMEORIGIN` would blank the Supercomputer Design-mode iframe, which
loads the live website from a cross-origin `higgsfield.app` host. Framing is
...
| Hardcoded string resembling a key (`sk-`, `ghp_`, `Bearer`) | Secret in source code / git history | `website_secrets` (set) when supported, or website settings; `bindings().SECRET_NAME` server-side |
...
After building any website, before `deploy_website`. Run alongside the SEO audit (`references/seo.md#audit`) as the security half of the pre-deploy quality gate. This audit applies to every site -- brochure, website, dashboard -- because even static sites can have XSS or misconfiguration.
