...
an **animated website** — the scroll-scrub journey (A4), by default a single
continuous film scrubbed by scroll — and
...
- **B1 — Cutout parallax rig.** Take the hero image, `remove_background` the
  subject, generate/outpaint a clean background plate behind it (+ optionally
...
- **B2 — Grade-shift interaction pair.** Two renders of the SAME composition
  ...
  effect.
- **B3 — 3D subject scene.** Approved hero image → `generate_3d` GLB → R3F
  scene with scroll-driven camera orbit + cursor tilt. Spectacle tier.
