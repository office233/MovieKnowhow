...
| Modern SaaS where you own the components | shadcn/ui (`bunx shadcn@latest add ...`) | You own the code, easy to customise; never ship default state |
...
### 3.A Stack
* **Framework:** **React 19 + TanStack Start** — the pinned platform stack (SSR
  on one Cloudflare Worker; see `references/runtime-and-infra.md`). No Next.js,
  Remix, or Astro — ignore any Next-specific advice (`next/font`, `next/image`,
  RSC directives) you know from elsewhere.
  * **SSR SAFETY:** every route renders on the server per request. Never touch
    `window`/`document`/`localStorage` at module top level or during render —
    only inside `useEffect`/event handlers or behind `typeof window !== "undefined"`.
  * **INTERACTIVITY ISOLATION:** any component using Motion, scroll listeners, or pointer physics MUST be an isolated SSR-safe leaf (effects/handlers only); the shell renders static layout.
* **Styling:** **Tailwind v4** (default). Tailwind v3 only if the existing project demands it.
  * For v4: do NOT use `tailwindcss` plugin in `postcss.config.js`. Use `@tailwindcss/postcss` or the Vite plugin.
* **Animation:** **Motion** (the library formerly known as Framer Motion). Import from `motion/react` (`import { motion } from "motion/react"`). The `framer-motion` package still works as a legacy alias - prefer `motion/react` in new code.
* **Fonts:** Self-host with `@font-face` + `font-display: swap` (or `@fontsource/*` packages). Never link Google Fonts via `<link>` in production.
...
* **LCP** < 2.5s. Hero image must be preloaded (`<link rel="preload" as="image">` in the route `head()`).
...
3. **Code sketch** - minimal working implementation (SSR-safe by default, motion isolated in a leaf component).
...
bun add @fluentui/web-components @fluentui/tokens

# IBM Carbon
bun add @carbon/react @carbon/styles
...
bunx shadcn@latest add button card badge separator input

# Primer CSS (GitHub product/devtool UI)
bun add @primer/css

# Primer Brand (GitHub marketing UI)
bun add @primer/react-brand

# GOV.UK Frontend
bun add govuk-frontend

# USWDS (US Web Design System)
bun add uswds

# Atlassian Design System (Atlaskit)
bun add @atlaskit/css-reset @atlaskit/tokens @atlaskit/button @atlaskit/badge @atlaskit/section-message @atlaskit/card
