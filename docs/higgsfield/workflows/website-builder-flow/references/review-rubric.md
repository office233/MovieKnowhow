...
9e. **Animated website — A4 scroll scrub (every website by default; skip only if
   ...
   first-frame poster extracted from the exact deployed clip; chapter copy is
   server-rendered in semantic document flow (not hidden until a viewport
   ...
   initialization runs only in an effect; and teardown aborts fetches, removes
   listeners/video nodes, cancels RAF, and revokes Blob URLs. Scrub videos
   directly from seekable MP4/Blob URLs, ensure CSP `media-src` permits `blob:`,
   and ensure no second ScrollTrigger timeline drives the same media.
   `multi-leg` only: connector/leg handoffs use the neighboring rendered clips'
   ACTUAL boundary frames and camera velocity does not reverse accidentally —
   read the seam frames, since there is no browsing step (the sandbox browser
   cannot reach a local preview). The engine ships pre-built in the
   `scroll-scrub` template, so these runtime properties are inherited, not
   re-implemented — check that the engine was NOT rewritten or replaced.
9f. **Animation mode gate (machine-verifiable — HARD, every website).** This is
   ...
   2. If the value is **`animated-website`**, ALL of these must hold, or FAIL:
      ...
      - real film shipped — at least ONE clip under
        `app/public/assets/**/*.mp4` (`single-shot` ships exactly one; a
        `multi-leg` journey ships one per leg), each with a first-frame poster
        and a mobile encode (item 9e);
      - no `<...>` placeholder survives in
        `app/src/scroll-scrub-scenes.ts`, and every clip/poster path it names
        exists on disk;
      - the brief carries the journey block (Journey shape + the chapters)
        written in Phase 0.
      A website on this path with no scroll-scrub component or no film is the
      failure mode this gate exists to catch — do NOT deploy it; go build the
      journey.
