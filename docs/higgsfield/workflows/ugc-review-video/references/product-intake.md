...
- `product_reference` — a confirmed attachment or imported hero-image `media_id`, ready for
  `medias[].value`
...
Bring a local photo into Higgsfield with `media_upload_widget` in an Apps UI-capable client.
When the client can provide bytes directly, use `media_upload`, PUT the bytes to its
`upload_url`, then call `media_confirm`. Keep the confirmed `media_id` as
`product_reference`.
...
- Import the first usable authorized HTTPS hero image with `media_import_url` and keep the
  returned `media_id` as `product_reference`. Import at most four additional same-SKU URLs only
  when later prompts genuinely need another angle.
