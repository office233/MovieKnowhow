...
> **Game asset reference.** Use for ANIMATED (rigged, multi-clip)
...
> **Use the native 3D tool first.** If `higgsfield_generate_3d` (supercomputer)
> or `generate_3d` (MCP) is loaded, it covers the whole image→3D→rig→animation
...
> `higgsfield_3d_animation_actions`. Billed in workspace credits — **no
...
| Fully automated 2D→animated GLB, best quality, paid | **Native 3D tool — preferred**: `higgsfield_generate_3d` model `image_to_3d` with `should_texture:true, enable_rigging:true, enable_animation:true, pose_mode:'a-pose'` (one clip per job; extra clips via model `3d_rigging` on the result GLB, ~8 credits each; clip ids via `higgsfield_3d_animation_actions`) → merge clips locally. **Before ANY image→3D submit read `meshy-input-rules.md`** (single-figure rule, character-sheet cropping, low-poly params — they apply to the native tool too). 38 workspace credits/character + 8/extra clip; no API key. Raw-API fallback (no native tool only): `meshy-api.md` |
...
- `higgsfield_upload` REJECTS `.glb` ("Invalid or blocked file extension") —
