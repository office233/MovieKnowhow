...
Wait on the returned indexed job with `jobs_wait({jobs:[{index,job_id}],timeout_seconds:15})`
until terminal, then keep `(character_media_id, character_url)` = `(job_id, result_url)`. That
identity is reused by every board and every clip and is never
...
The parent skill's creator-authorization gate has precedence. Never construct a
recognizable public figure, celebrity, minor, or non-consenting real person. A
text description must produce an original fictional adult; a supplied identity
reference is legal only after the parent gate establishes authorization.

**1. Age enforcement — minimum 21 years old.**
If `user_overrides` requests a child, minor, or teenager, or otherwise leaves
adult status unclear, stop and return to the parent skill's authorization gate.
Do not compose a character prompt and do not silently age-transform the request.
