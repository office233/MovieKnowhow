...
`generate_image` (strong model: `gpt_image_2` or `nano_banana_pro`).
...
Submit all boards as async jobs at once, keep working while they render, poll
`job_display`, download into the repo's `refs/` directory (working
artifact — NOT `app/public/`).
