...
The file path is `robots.txt.ts` — TanStack Start maps the `.txt` extension to serve at `/robots.txt`. Origin is derived from the request so it works at the deployed host without hardcoding.
...
3. **robots.txt with a hardcoded origin.** The route derives origin from the request URL, so the deployed site gets correct sitemap URLs automatically. Don't hardcode URLs.
...
Run this audit after building any website and before calling `deploy_website`. It is the SEO quality gate. Do not deploy until every FAIL is resolved. The audit is self-contained — read the project source files directly, no external tools or browser needed.
...
5. Only after a clean pass (zero FAILs), proceed to `deploy_website`.
