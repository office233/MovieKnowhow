#!/usr/bin/env python3
"""Build the canonical Brandkit PPTX and matching verified PDF.

This is the sandbox-script port of the former brandkit_brandbook_builder tool.
It edits the fixed OOXML template directly, preserves its geometry, consumes
only separately approved Brandkit state, and leaves publishing to the
media_upload / media_confirm tools.
"""
...
from brandkit import assert_public_host, normalize_hex, read_state, slug
...
def convert_png(data: bytes, width: int, height: int, fit: str) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix="brandkit-image-") as directory:
        ...
        if suffix == ".svg":
            ...
            if not rsvg_convert:
                raise RuntimeError(
                    "rsvg-convert is unavailable in the managed sandbox"
                )
...
def resolve_fonts(choice: dict[str, Any]) -> list[tuple[str, bytes]]:
    ...
    if source.startswith("google:"):
        ...
        raise RuntimeError(
            f"approved font '{family}' requires a public font URL or google: source"
        )
...
def place_mockups(
    ...
    for pair in range((len(mockups) + 1) // 2):
        ...
        entries[f"ppt/media/brandkit-mockup-{first_index + 1}.png"] = convert_png(
            source_bytes(str(mockups[first_index]["url"]), MAX_ASSET_BYTES),
            1200,
            1600,
            "cover",
        )
        if second_index < len(mockups):
            ...
            entries[f"ppt/media/brandkit-mockup-{second_index + 1}.png"] = convert_png(
                source_bytes(str(mockups[second_index]["url"]), MAX_ASSET_BYTES),
                1200,
                1600,
                "cover",
            )
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not logo_source:
        raise RuntimeError("approved logo requires a permanent URL or sandbox path")
