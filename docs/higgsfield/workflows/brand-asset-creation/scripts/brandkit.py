#!/usr/bin/env python3
"""Deterministic Brandkit operations for the per-user E2B sandbox.

This script deliberately uses only the Python standard library plus the
ImageMagick and librsvg CLIs present in the mcp-sandbox template. It owns
Brandkit's approval ledger, editable HTML review boards, and
geometry-preserving SVG/PNG logo exports. Publishing files remains the job of
the media_upload / media_confirm tools.
"""
...
def normalize_asset(value: Any) -> dict[str, Any]:
    ...
    if not asset_id:
        raise RuntimeError("Brandkit asset.id is required")
    result = {"id": asset_id}
    for key in ("file", "type", "url"):
...
def run_state(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if action == "get_essential_kit":
        return {
            "approved": state.get("essential_kit_approved", False),
            "approved_slots": {
    ...
    if action == "clear":
        ...
        result = {
            "invalidated_brandbook_elements": invalidated,
            **(
                {"invalidated_generated_logo": None}
                if slot in {"palette", "typography"}
                else {}
            ),
    ...
    elif action == "approve_essential_kit":
        summary = str(payload.get("approval_summary", "")).strip()
        if not summary:
            raise RuntimeError("input requires approval_summary")
        for slot in ("logo", "palette", "typography"):
            current = state.get(slot)
            incoming = normalize_slot(require_object(payload, slot), slot)
            if isinstance(current, dict) and current.get("origin") == "user_supplied":
                if slot == "logo":
                    current_asset = current.get("asset", {})
                    incoming_asset = incoming.get("asset", {})
                    if current_asset.get("id") != incoming_asset.get("id"):
                        raise RuntimeError(
                            "approve_essential_kit cannot replace the user-supplied official logo"
                        )
                    continue
                comparable = {
                    key: item
                ...
                if comparable != incoming:
                    raise RuntimeError(
                        f"approve_essential_kit cannot replace the user-supplied official {slot}"
                    )
        invalidated = invalidate_brandbook(state)
        old_logo_name = (
            state["logo"].get("name")
            if isinstance(state.get("logo"), dict)
            else None
        )
        for slot in ("logo", "palette", "typography"):
            current = state.get(slot)
            origin = (
                ...
                and current.get("origin") == "user_supplied"
                else "brandkit_generated"
            )
            value = normalize_slot(require_object(payload, slot), slot)
            value["approvalSummary"] = summary
            value["origin"] = origin
            value["revision"] = next_revision(current)
            state[slot] = value
        axes = payload.get("visual_axes")
        if isinstance(axes, dict):
            state["visualAxes"] = {
                "familiarExperimental": axes["familiar_experimental"],
                "geometricOrganic": axes["geometric_organic"],
            ...
            state.setdefault(
                "visualAxes",
                {
                    "familiarExperimental": 50,
                    "geometricOrganic": 50,
                    "restrainedExpressive": 50,
                },
            )
        state["version"] = int(state.get("version", 0)) + 1
        result = {
            "invalidated_brandbook_elements": invalidated,
            "invalidated_logo": old_logo_name,
            "revisions": {
                slot: state[slot]["revision"]
                for slot in ("logo", "palette", "typography")
            },
            "saved": "essential_kit",
            "version": state["version"],
...
def write_logo_pair(
    ...
    if not image_magick:
        raise RuntimeError("ImageMagick is unavailable in the managed sandbox")
    if not rsvg_convert:
        raise RuntimeError("rsvg-convert is unavailable in the managed sandbox")
...
def css_string(value: str) -> str:
    # Escape CSS strings and HTML raw-text delimiters without changing Unicode names.
    return '"' + "".join(
        "\ufffd" if char == "\0" else
...
def font_css(font: dict[str, Any], default_family: str, default_weight: int = 400) -> tuple[str, str]:
    family = str(font.get("family", default_family))
    source = str(font.get("source", ""))
    weight = int(font.get("weight", default_weight))
    if source.startswith("google:"):
        google_family = source.removeprefix("google:").strip() or family
        encoded = quote_plus(google_family)
        css = (
            "@import url("
            + css_string(f"https://fonts.googleapis.com/css2?family={encoded}:wght@{weight}&display=swap")
            + ");"
        )
...
def logo_markup(source: str | None) -> str:
    if not source:
        return ""
    svg = read_svg(source)
    attributes, body, _ = validate_svg(svg)
    if not re.search(r"\sxmlns\s*=", attributes):
        svg = f'<svg xmlns="http://www.w3.org/2000/svg"{attributes}>{body}</svg>'
    # Image context keeps SVG active content out of the review document.
    encoded = base64.b64encode(svg.encode("utf-8")).decode("ascii")
    return f'<img class="logo" src="data:image/svg+xml;base64,{encoded}" alt="Selected logo">'
...
def build_review_html(review: dict[str, Any]) -> str:
    ...
    display_css, display_family = font_css(
        review.get("display_font", {}), "Arial", 700
    )
    body_css, body_family = font_css(review.get("body_font", {}), "Arial")
    display_weight = int(review.get("display_font", {}).get("weight", 700))
    body_weight = int(review.get("body_font", {}).get("weight", 400))
    display_stack = css_string(display_family) + ",sans-serif"
    body_stack = css_string(body_family) + ",sans-serif"
    ...
    note = {
        "palette": "Editable palette draft. Colors remain changeable until selection.",
...
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--text);font-family:{body_stack};font-weight:{body_weight}}}
...
h1{{margin:9px 0 6px;font:{display_weight} 38px/1.05 {display_stack}}}.premise{{max-width:700px;margin:0;font-size:16px;line-height:1.45}}
...
.specimen{{margin-top:28px;padding-top:22px}}.headline{{margin:8px 0 0;color:var(--text);font:{display_weight} 52px/1.05 {display_stack}}}
.body-copy{{max-width:760px;margin:14px 0 0;font:{body_weight} 18px/1.4 {body_stack}}}.ideas{{margin:0;padding-left:18px}}.ideas li+li{{margin-top:6px}}
...
def run_preview(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for review in reviews:
        ...
        if stage == "palette":
            if not 2 <= len(palette) <= 8 or not isinstance(
                logo_ideas, list
            ) or len(logo_ideas) not in {0, 2, 3}:
                raise RuntimeError(
                    f"palette review '{review.get('name', '')}' requires 2-8 palette colors and either no logo_ideas or 2-3 logo_ideas"
                )
    ...
    return {
        "files": files,
        "next": "Publish these paths per references/inline-widgets.md: use the pre-reserved uploads to screenshot and PUT PNG+HTML before this sandbox call ends, then media_confirm and show the review with HTML links.",
    }
