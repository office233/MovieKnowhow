...
A successful upload or a text-only `media_confirm` response is not visual evidence. Inspect a rendered image through an available image-viewing surface. If the host cannot show the actual pixels, disclose that visual QA remains unverified; do not claim it passed.
...
When logo, typography, or palette approval changes, remove every invalidated brandbook element from the active deliverable set. Rebuild and re-approve those elements; never silently keep assets tied to older revisions.

When typography or palette changes, also discard the prior approved logo and run the three-candidate logo approval step again.
